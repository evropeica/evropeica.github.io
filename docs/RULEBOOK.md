...
| S2 | Pages may use v2.0 vocabulary (every such word must be in `data/v2-lexicon.json`). **Stress marks are not used in running text.** They appear only where stress is the subject: the v2.1 section of `/spec/v2/`, and standalone letter specimens elsewhere (the `á é ý` list in the version table, the dead-acute rows on the keyboard page, `Ḯ` in the alphabet strip). Marking stress by hand across ordinary prose cannot be verified until the stress dictionary exists, and a wrong mark on a site about Ukrainian orthography is worse than no mark. The one exception is the project's name, **Evropéïća**, which keeps its acute as a fixed proper form (S5). The checker strips accents before comparing, so it cannot catch a misplaced stress; that is exactly why running text carries none. |
